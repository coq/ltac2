...
let perform_eval ~ontop e =
  ...
  let selector, proof =
    ...
    | Some pstate ->
      ...
      Lemmas.pf_fold Proof_global.give_me_the_proof pstate
  in
...
let register_struct ?local ~ontop str = match str with
...
let solve ~ontop default tac =
  let ontop, status = Lemmas.with_current_proof begin fun etac p ->
    ...
    let p = Proof.maximal_unfocus Vernacentries.command_focus p
    in
    p, status
    end ontop in
